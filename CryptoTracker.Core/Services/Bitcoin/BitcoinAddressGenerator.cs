...
namespace CryptoTracker.Core.Services.Bitcoin;
...
public class BitcoinAddressGenerator
{
    ...
    {
        ValidateIndex(index);
        var address = _extKey.Derive(0).Derive((uint)index).PubKey.GetAddress(type, Network.Main);
    ...
    {
        var data = Encoders.Base58Check.DecodeData(zpub);
        data[0] = 0x04;
        data[1] = 0x88;
        data[2] = 0xB2;
        data[3] = 0x1E;
